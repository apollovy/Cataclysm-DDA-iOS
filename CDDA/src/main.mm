...
#if defined CDDA_IOS
    PATH_INFO::init_user_dir( [documentPath cStringUsingEncoding:NSASCIIStringEncoding] );
#else
    PATH_INFO::init_user_dir("./");
#endif // CDDA_IOS
#   endif // USE_HOME_DIR
    ...
    PATH_INFO::set_standard_filenames();
#ifdef CDDA_IOS
    PATH_INFO::set_datadir( [[basePath stringByAppendingString:@"data/"] cStringUsingEncoding:NSASCIIStringEncoding] );
    PATH_INFO::set_savedir( [[documentPath stringByAppendingString:@"save/"] cStringUsingEncoding:NSASCIIStringEncoding] );
//    PATH_INFO::update_pathname("templatedir", [[documentPath stringByAppendingString:@"templates/"]cStringUsingEncoding:NSASCIIStringEncoding] );
    PATH_INFO::set_memorialdir([[documentPath stringByAppendingString:@"graveyard/"] cStringUsingEncoding:NSASCIIStringEncoding] );
...
//    PATH_INFO::update_pathname("mods-user-default", [[documentPath stringByAppendingPathComponent:@"user-default-mods.json"] cStringUsingEncoding:NSASCIIStringEncoding] );
    ...
    if( ![[NSFileManager defaultManager] fileExistsAtPath:userOptionsFilePath] )
    {
        ...
        NSString* defaultOptionsFilePath;
        if ( ( [SDVersion deviceSize] == Screen4inch ) || ( [SDVersion deviceVersion] == iPhone5 ) || ( [SDVersion deviceVersion] == iPhone5S ) )
        {
            defaultOptionsFilePath = [basePath stringByAppendingPathComponent:@"config/options_32.txt"];
        }
        else if ( ( [SDVersion deviceSize] == Screen3Dot5inch ) || ( [SDVersion deviceVersion] == iPhone4S ))
        {
            defaultOptionsFilePath = [basePath stringByAppendingPathComponent:@"config/options_24.txt"];
        }
        ...
        {
            defaultOptionsFilePath = [basePath stringByAppendingPathComponent:@"config/options_48.txt"];
        }
        ...
        {
            defaultOptionsFilePath = [basePath stringByAppendingPathComponent:@"config/options_64.txt"];
        }
        else
        {
            defaultOptionsFilePath = [basePath stringByAppendingPathComponent:@"config/options_32.txt"];
        ...
        [[NSFileManager defaultManager] copyItemAtPath:defaultOptionsFilePath toPath:userOptionsFilePath error:&e];
    }
    PATH_INFO::set_options([userOptionsFilePath cStringUsingEncoding:NSASCIIStringEncoding]);
